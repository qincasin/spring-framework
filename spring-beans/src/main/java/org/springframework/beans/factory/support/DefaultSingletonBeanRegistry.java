...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	...
	/** Cache of singleton objects: bean name to bean instance.一级缓存 对象初始化完成后都会放置到一级缓存中 二级和三级都是为了解决循环依赖的*/
	...
	protected Object getSingleton(String beanName, boolean allowEarlyReference) {
		...
		if (singletonObject == null && isSingletonCurrentlyInCreation(beanName)) {
			//再次去二级缓存里面拿 如果二级缓存中拿不到 且 存在早期引用的 继续往下走
					...
					if (singletonObject == null) {
						...
						if (singletonObject == null) {
							...
							if (singletonFactory != null) {
								singletonObject = singletonFactory.getObject();//拿早期实例
								//想二级缓存存放数据
								this.earlySingletonObjects.put(beanName, singletonObject); //缓存升级，放到二级缓存中
								//将3级缓存的数据清除
								this.singletonFactories.remove(beanName);  // 三级缓存中清除
							}
						}
					}
				}
	...
	public Object getSingleton(String beanName, ObjectFactory<?> singletonFactory) {
		...
		synchronized (this.singletonObjects) {
			Object singletonObject = this.singletonObjects.get(beanName);
			if (singletonObject == null) {
				//容器销毁时，会设置换这个属性为true，这个时候就不能在创建bean实例了，直接报错
				...
				//这个方法会将bena记录到 singletonsCurrentlyInCreation Set集合中
				//将当前beanName放入到"正在创建中单实例集合"，放入成功说明没有产生循环依赖，失败，则产生循环依赖，里面会抛出异常
				/** Names of beans that are currently in creation. */
//				private final Set<String> singletonsCurrentlyInCreation =
//						Collections.newSetFromMap(new ConcurrentHashMap<>(16));
//
				//举个例子 构造方法参数依赖
				// A--->B  B--->A
				//1.加载A 根据A的构造方法，想要去实例化A，但是发现A的构造方法里面有一个参数是B(在这之前已经先这个集合中添加了{A})
				//2.因为A的构造方法依赖了B，所以要去加载B
				//3.加载B,根据B的构造方法，想要去实例化B，但是发现B的构造方法里面有个参数是A(在这个之前已经向这个集合中添加B)
				//4.因为B的构造方法依赖了A，所以要去加载A
				//4.再次来getSingleton方法里面，调用beforeSingleCreation(A),发现集合里面已经有A了，所以添加失败，抛出异常
				//完事
				beforeSingletonCreation(beanName); //循环依赖相关
	...
	public void registerDependentBean(String beanName, String dependentBeanName) {
		...
		//dependenciesForBeanMap 处理 处理A依赖的谁的集合
	...
	public void destroySingletons() {
		...
		synchronized (this.singletonObjects) {
			//设置该属性，表示当前beanFactory状态转变为了 销毁状态
			this.singletonsCurrentlyInDestruction = true;
		}

		//disposable ? 创建单实例时，会检查当前单实例是否实现了 DisposableBeans 接口，如果实现了该接口，对应的容器销毁时，需要执行该bean.destroy()方法
	...
	public void destroySingleton(String beanName) {
		...
		//清空三级缓存 + registeredSingletons里面对应当前BeanName的数据
	...
	protected void destroyBean(String beanName, @Nullable DisposableBean bean) {
		...
		//谁依赖了当前bean，那么依赖的那些bean 以及当前都要去调用析构函数去销毁
		//dependentBeanMap ? 保存的是 依赖当前bean的其他bean信息
		//eg: b/c/d 这三个bean依赖了a，那么dependentBeanMap 内部结构为：
		//{key:a,value:{b,c,d}}
		synchronized (this.dependentBeanMap) {
		...
		if (dependencies != null) {
			...
			for (String dependentBeanName : dependencies) {
				//因为依赖对象要被回收了，所以依赖当前 bean的 其他对象，都要执行 destroySingleton 逻辑
		...
		// dependenciesForBeanMap 是保存当前bean都依赖了谁
		//eg: 比如a 依赖了 w，x，那么 dependenciesForBeanMap 中就有如下的数据信息：
		//{a:{w,x}}
